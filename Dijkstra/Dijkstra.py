    ...
    def variarRed(self):
        ...
        for i in range(0,len(RED)):
            ...
            for j in range(0,len(RED)):
                ...
                elif nuevoValor == 0 or nuevoValor == 99 or nuevoValor == 100:
                    RED[i][j] = INF
                    RED[j][i] = INF
                else:
                    RED[i][j] = nuevoValor
    ...
    def Algoritmo(self, fin):
        ...
        for i in range(0, len(self.definitivos)-1):
            if self.definitivos[i] != "BORRAR" and self.definitivos[i+1] != "BORRAR":
                ...
                if RED[VER[self.definitivos[i]]][VER[self.definitivos[i + 1]]] == 0:
                    self.definitivos[i] = "BORRAR"

                elif RED[VER[self.definitivos[i]]][VER[self.definitivos[i+1]]] == INF:
                    #self.definitivos[i] = "BORRAR"
                    #aux = self.definitivos[i + 1]

                    if self.definitivos[i] != aux:
                        self.definitivos[i] = "BORRAR"
                    else:
